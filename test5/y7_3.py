...
LLM_MODEL_NAME = "gemini-2.5-flash" # 안정적인 모델로 변경
OUTPUT_DIR = "output"
...
class StoryAnalyzer:
    ...
    def analyze(self, chunk: Dict) -> Dict:
        ...
        '인물' 분류 시 작가, 저자, 책 제목은 절대 포함하지 마세요.
...
class WikiGenerator:
    ...
    def save_report_to_file(storyboard_list: List[Dict]):
        ...
        # 1. 소설 기본 정보 추출
        book_title = storyboard_list[0].get('book_info', {}).get('title', 'Unknown Title')
        author = storyboard_list[0].get('book_info', {}).get('author', 'Unknown Author')
        ...
        for scene in storyboard_list:
            ...
            for ent in scene.get('entities', []):
                # 작가나 책 제목이 인물로 들어온 경우 필터링
                if ent['name'] in [book_title, author]: continue
        ...
        with open(file_path, "w", encoding="utf-8") as f:
            ...
            for scene in storyboard_list:
                f.write(f"- **[{scene['scene_id']}] {scene.get('scene_title','')}**\n")
...
def main():
    ...
    # 1. 청킹
    chunks = HybridSceneChunker().split_content(open(INPUT_FILE_NAME, 'r', encoding='utf-8').read())
