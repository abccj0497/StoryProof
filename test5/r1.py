...
OUTPUT_DIR = "output_v2"  # v2 폴더로 변경
...
# =========================================================
# [PART 1] 하이브리드 청커 V2 (Sliding Window 추가)
# =========================================================
class HybridSceneChunker:
    ...
    def __init__(self, target_chars=3500, min_chars=1000, threshold=5, overlap_chars=200):
        ...
        self.overlap_chars = overlap_chars # 문맥 유지를 위한 겹치기 구간
        ...
        for para in paragraphs:
            ...
            if (is_chapter and current_len > 0) or \
               ...
               (current_len + len(para) > self.target_chars):
                ...
                # 현재 씬 저장 (앞에 buffer_text가 있다면 포함되었을 것임)
                full_scene_text = "\n\n".join(current_scene)
                final_scenes.append(full_scene_text)
                
                # [V2 핵심] 다음 씬을 위해 현재 씬의 마지막 일부(overlap)를 버퍼로 잡음
                buffer_text = full_scene_text[-self.overlap_chars:] if len(full_scene_text) > self.overlap_chars else full_scene_text
                
                # 새 씬 시작 (버퍼 내용은 AI 분석용으로만 참고하고, 실제 텍스트 저장엔 중복 최소화가 좋지만, 여기선 문맥이 중요하므로 포함)
                # *단, 파일 저장시엔 중복되어 보일 수 있으나 분석 품질엔 유리함.
                current_scene = [para] 
...
# =========================================================
# [PART 2] 스토리보드 추출기 V2 (감정선, 키워드 추가)
# =========================================================
class StoryAnalyzer:
    ...
    def analyze(self, chunk: Dict) -> Dict:
        ...
        {{
          ...
          "scene_title": "장면 소제목 (매력적으로)",
          "summary": "장면 요약 (3-5문장)",
          "atmosphere": "장면의 분위기 (예: 긴장감 넘치는, 평화로운, 슬픈)",
          "keywords": ["키워드1", "키워드2", "중요단어"],
          "entities": [
    ...
    def save_report_to_file(storyboard_list: List[Dict]):
        ...
        with open(file_path, "w", encoding="utf-8") as f:
            ...
            if not char_items:
                ...
                f.write("| 이름 (Name) | 주요 특징 (Traits) | 주요 행동 (Actions) | 등장 씬 |\n")
                f.write("|---|---|---|---|\n")
                for name, details in char_items.items():
                    # 정보 병합 (너무 길어지면 잘라서 보여줌)
            ...
            for key in ["Item", "Place"]:
                ...
                for name, details in items.items():
                    ...
                    desc_str = desc_set[0] if desc_set else "설명 없음"
                    f.write(f"- **{name}**: {desc_str}\n")
                f.write("\n")
...
def main():
    ...
    for i, chunk in enumerate(scene_data):
        ...
        time.sleep(1.2) # API 제한 고려하여 약간의 딜레이
