        ...
        for idx in top_indices:
            ...
            if p_id not in seen_parents:
                results.append({
                    ...
                    "parent_id": p_id,
                    "scene_id": child['metadata']['scene_id'], # 👈 결과 확인용 scene_id
...
def calculate_metrics(db, eval_dataset: List[Dict], k_values=[1, 3, 5]):
    ...
    for i, item in enumerate(eval_dataset):
        ...
        if i < 1:
            ...
            print(f"   -> 정답 Scene ID: {item.get('target_scene_id', 'Unknown')}")
...
# ==========================================
# 🚀 4. 메인 실행 (순차적 번호 부여 최종 수정본)
# ==========================================
if __name__ == "__main__":
    # 파일 설정 (테스트용)
    FILE_NAME = "test_novel.txt"
    if not os.path.exists(FILE_NAME):
        dummy_text = "앨리스는 강둑에 앉아 있었다. " * 300
        with open(FILE_NAME, "w", encoding='utf-8') as f: f.write(dummy_text)
    ...
    # 시간 절약을 위해 3개 청크만 처리 (전체는 parents[:3] -> parents 로 수정)
    process_limit = 3

    # 전체 청크를 순서대로 처리합니다.
    for i, p_text in enumerate(parents[:process_limit]):
        print(f"   -> Chunk {i+1} 처리 중... (현재 Scene 번호: {global_scene_counter}부터 시작)")
